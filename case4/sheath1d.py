...
The Sheath1D class models the ion formation and transport in a sheath
over a flat absorbing surface with a semi-infinite flow stagnating from
above.  This simulation is intended to model the electrical behavior in 
a flame stagnating on a metal surface.

The length scale, z, is scaled logarithmically, so that physical 
coordinates may be calculated as
    x = - gamma * lambda * ln ( 1-z )
where lambda is the Debye length, and gamma is a numerical domain 
compression factor, which is used adjust the "zoom" on the sheath.  In
this way, when x->infty, z->1, regardless of the choice of gamma,  but
by choosing a large gamma, features that might be "bunched up" near the
z=1 location can be resolved more clearly by "zooming out" on the scale.

The free stream velocity intensity is set dimensionlessly by the ionic
Reynolds number, R, and it is presumed to approach zero at the surface.
The "alpha" parameter sets the number of Debye lengths over which the
velocity declines with an exponential function.

There is also presumed to be formation of secondary ions near the metal
surface.  The dimensionless intensity of the formation rate is set by 
"omega" and the length scale over which these reactions are presumed to
occur is set by "beta."  Omega and beta are derived such that if they 
are both equal to one, the dimensionless secondary ion concentration 
should be roughly equal to 1.

psia determines the dimensionless applied electric field in the 
farfield. It also determines the electrical current being conducted 
through the distant neutral plasma, so it is equivalent to setting the 
net electrical current.

For constant-voltage simulations, see the Sheath1DV class.
"""
...
class Sheath1DV(Sheath1D):
    """The Sheath1DV class models the sheath in a stagnation problem
    
This is avariant of the Sheath1D model with a constant-voltage condition
...
phia determines the dimensionless applied electric voltage in the 
farfield. Since finite currents in the uniform plasma produce uniform 
electric fields, there is no uniform plasma potential.  Instead, this
model calculates the voltage drop across the sheath as the integral of
the electric field subtracted by the farfield electric field, producing
a finite value.

For constant-current simulations, see the Sheath1D class.
"""
...
mu          Negative-to-positive species mobility ratio (200.)
tau         Negative-to-positive temperature ratio (1.)
phia        Voltage drop in the sheath (1.)
omega       Dimensionless formation rate in the sheath (0.)
